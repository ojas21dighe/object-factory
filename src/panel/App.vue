...
export default {
  ...
  methods: {
    ...
    // highlightAllMatches() {
    // console.log("Locators highlight", this.model);  
    
    // for (i = 0; i < this.model.locators.length; i += 1) {
    //   console.log("Current Locator: ", this.model.locators[i]);
    //   currentLocator = this.model.locators[i];
    //   if (!currentLocator.hidden) {
    //     const matches = locatorMatches(currentLocator);
    //     if (currentLocator.locator && matches.length === 1) {
    //       chrome.runtime.sendMessage({ type: 'appHighlightAllMatches', data: {locator: currentLocator.locator} });
    //     }
    //   }
    // }
    // },
