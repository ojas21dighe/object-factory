<template>
  <v-app-bar dense fixed>
   ...
   <v-tooltip bottom :disabled="!showTooltips || isScanning" open-delay="600">
      ...
      <span v-if="!isInspecting">Add Single Object</span> <span v-if="isAdding">Stop Adding Element</span>
    </v-tooltip>

    <v-tooltip bottom :disabled="!showTooltips || isAdding" open-delay="600">
      <template v-slot:activator="{ on }">
      ...
      <span v-if="!isInspecting">Scan Page/ Area</span> <span v-if="isScanning">Stop Scanning</span>
    </v-tooltip>

    <!-- <v-tooltip bottom :disabled="!showTooltips || !hasModel || isInspecting" open-delay="600">
      <template v-slot:activator="{ on }">
        <v-btn v-on="on" icon :disabled="!hasModel || isInspecting" v-on:click="highlightAll">
          <v-icon> {{ mdiEye }} </v-icon>
        </v-btn>
      </template>
      <span>Highlight Matched Elements</span>
    </v-tooltip> -->
    ...
    <v-tooltip bottom :disabled="!showTooltips || (!hasModel && !isInspecting)" open-delay="600">
      ...
      <span>Generate Page Object Model</span>
    </v-tooltip>
